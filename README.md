...
- **Provisions** -- structured legal provisions per jurisdiction, tagged by topic, with article references and YAML frontmatter linking to primary sources
...
- **Citations** -- central registry of 36 primary sources across 7 jurisdictions (`knowledge/citations.json`)
- **Enforcement** -- 30 enforcement cases with verification fields and decision-specific URLs
...
The knowledge base content was initially generated by an LLM. To prevent
hallucinated legal provisions from reaching users, every claim is tracked
through an audit trail:

- **YAML frontmatter** on all 41 provision files links each file to primary
  source IDs in `knowledge/citations.json`, with verification status tracking
  (`unverified` -> `in-review` -> `verified`)
- **Claim extraction** -- 483 verifiable claims (article numbers, dates,
  thresholds, fine amounts) extracted and tracked in `audit/extracted-claims.json`
- **URL validation** -- all source URLs checked for accessibility; generic
  homepage URLs flagged for replacement with decision-specific links
- **CI checks** -- PRs touching `knowledge/` trigger frontmatter validation,
  claim extraction, and enforcement case schema checks
- **Weekly cron** -- scheduled URL validation auto-creates GitHub Issues
  if any source links break

See [knowledge/CONTRIBUTING.md](knowledge/CONTRIBUTING.md) for contribution
requirements and verification workflow.
...
knowledge/
  ...
  citations.json  Central primary source registry (36 sources)
  CONTRIBUTING.md Contribution and verification guidelines
  snapshots/      Versioned regulatory snapshots

scripts/
  verify-urls.ts          URL validator (CI-compatible)
  extract-claims.ts       Claim extractor (483 verifiable claims)
  generate-audit-report.ts  Combined audit report generator
  add-frontmatter.ts      Frontmatter addition script
  extend-enforcement-cases.ts  Enforcement case extension script
  fix-manifest-dates.ts   Manifest date correction script

audit/
  extracted-claims.json   All extracted verifiable claims
  audit-report.md/.json   Baseline audit report
  audit-log.json          Permanent audit trail
...
# Knowledge base verification
npx tsx scripts/extract-claims.ts       # Extract verifiable claims
npx tsx scripts/verify-urls.ts          # Check all source URLs
npx tsx scripts/generate-audit-report.ts # Generate full audit report
```
...
and add jurisdiction-specific nuance via pull requests. Every provision file
has YAML frontmatter linking to primary sources, and every factual claim
is tracked for verification. See
[knowledge/CONTRIBUTING.md](knowledge/CONTRIBUTING.md) for the contribution
