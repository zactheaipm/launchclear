...
const ARTIFACT_TYPE_TO_DEFAULT_TEMPLATE: Readonly<Partial<Record<ArtifactType, string>>> = {
	...
	"conformity-assessment": "ai-act-risk-assessment",
	"transparency-notice": "transparency-notice",
	"model-card": "model-card",
...
function generateFilename(templateId: string, jurisdictions: readonly string[]): string {
	const jurisdictionSuffix =
		jurisdictions.length === 1 ? jurisdictions[0] : "multi-jurisdiction";
...
export async function generateArtifacts(
	...
	// Process each deduplicated requirement
	...
	for (const result of results) {
		...
		const artifactType =
			TEMPLATE_ID_TO_ARTIFACT_TYPE[result.entry.templateId] ??
			result.entry.requirement.type;
